...
> 按「文件名关键词 → 微信昵称」的映射，批量把某个文件夹里的文件自动发给对应的微信联系人。
...
- **两种映射录入方式**：
  - 上传 Excel（关键词列 + 微信昵称列）；
  - 直接在文本框里按 `关键词,微信昵称` 逐行输入。
- **模板内嵌**：无需附带任何文件，界面点「下载模板」即可获取映射表模板（已编译进 exe 资源）。
- **实时预览**：扫描文件夹后列出每个文件将发往哪个联系人，发送前可核对。
- **可选附言**：发送文件前可填一条文字消息。
...
├── 发送映射模板.xlsx          # 关键词 / 微信昵称 模板（已编译进 exe，可单独下载）
...
2. **昵称精确匹配**：依赖微信搜索框能唯一命中昵称；若昵称含特殊字符或被搜索折叠，可能开错会话。
